...
class Image(Params):
    ...
    def blurImgs(self, imgs_list, kernelRadius):
        """
...
class Scene():
    ...
    def quantizePts2Cubes(self,pts_xyz, resol, cube_D, cube_Dcenter, cube_overlapping_ratio, BB = None, type_v = 'None'):
        """
        ...
        if(type_v is 'downSample'):
            pcd = o3.PointCloud()
            pcd.points = o3.Vector3dVector(pts_xyz)
            downpcd = o3.voxel_down_sample(pcd, voxel_size = cube_down_mm)
            pts_xyz = np.asarray(downpcd.points)
            print('downSample points num: ', pts_xyz.shape[0])
        ...
        if(True):
            ...
            if(type_v is 'downSample'):
                cubes_ijk = np.vstack([cubes_ijk_floor, cubes_ijk_floor])  # (2*N_pts, 3) 
                cubes_ijk = cubes_ijk_floor
                #cubes_ijk = np.vstack([cubes_ijk_floor, cubes_ijk_ceil])
            else:
                cubes_ijk = np.vstack([cubes_ijk_floor, cubes_ijk_ceil])  # (2*N_pts, 3)
...
class Camera():
    ...
    def readCameraP0_as_np_tanks(self, cameraPO_file):
    ...
    def readCameraPOs_as_np(self, 
        ...
        elif 'tanks' in datasetName:
            for _i, _view in enumerate(viewList):
                _cameraPO = self.readCameraP0_as_np_tanks(cameraPO_file = os.path.join(datasetFolder, poseNamePattern.replace('#', '{:03}'.format(_view)).replace('@', '{}'.format(_view))))
        ...
        elif 'eth3d' in datasetName:
            print('read eth3d')
            for _i, _view in enumerate(viewList):
                _cameraPO = self.readCameraP0_as_np_tanks(cameraPO_file=os.path.join(datasetFolder,
...
class PLY2volumn(Params):
    def __init__(self, model_num, cube_D = None):
        super(PLY2volumn, self).__init__()
        self.model_num = model_num
        
        self.load_modelSpecific_params(self._datasetName, model_num, cube_D = cube_D)
    ...
    def ply2array(self):
        ...
        if(self._datasetName == 'DTU'):
            pcd_name = os.path.join(self.datasetFolder, self.stl_comb_name)
...
class ViewPairSelection(Params):
    def __init__(self, selection_method = 'angle', thre_angle = 90):
        super(ViewPairSelection, self).__init__()
        self.camera = Camera()
        self.min_angle = 90
        self.view_pair_num = 5
        self.selection_method = selection_method
        self.thre_angle = thre_angle
        

    
    def argmaxN_viewPairs(self, viewPairs, w_viewPairs, features_viewPairs, N_argmax):
        """
        inputs:
        ...
        print(features_viewPairs.shape)
        print(argmaxN_w.shape)
        argmaxN_e = features_viewPairs[indice_cube, indice_N_max]
        return argmaxN_viewPairs, argmaxN_e
    ...
    def cal_w_viewPairs(self,
        ...
        print('thre_angle', thre_angle)
        valid_viewPair = theta_viewPairs < (thre_angle * np.pi /180.)
        print('selection_method',self.selection_method)
        if(self.selection_method == 'random'):
            angle_viewPairs = np.random.rand(num_validCubes, num_viewPair)
        elif(self.selection_method == 'angle'):
        ...
        elif(self.selection_method == 'angle_random'):
            print('changed 000')
            angle_viewPairs = np.zeros((num_validCubes, num_viewPair))
            for i in range(num_validCubes):
                dot = unit_vector(vect_p2p3)[i][None,...] * unit_vector(vect_p2p1)
                ...
                angle_viewPairs[i] = cos_angle
            angle_viewPairs = (angle_viewPairs < 0.7) 
            angle_viewPairs = angle_viewPairs * np.random.rand(num_validCubes, num_viewPair)

        score_viewPair = angle_viewPairs * valid_viewPair[:,:,0] 
        
        return score_viewPair

    def downsamplePts2P(self, pts_xyz, resol):
        pcd = o3.PointCloud()
        pcd.points = o3.Vector3dVector(pts_xyz)
        downpcd = o3.voxel_down_sample(pcd, voxel_size = resol)
        downppp = np.asarray(downpcd.points)
        return downppp

    def occlusion_detect(self, cameraTs_np, valid_cubeCenters_xyz, pts, resol = 0.3):
        
        radius = resol * 32
        N_validCubes = valid_cubeCenters_xyz.shape[0]
        N_viewPairs = cameraTs_np.shape[0]
        occlusion_viewPairs = np.empty((N_validCubes, N_viewPairs), dtype = np.float32)
        
        t_p = cameraTs_np
        pts = self.downsamplePts2P(pts, radius/2)
       
        for i in range(N_validCubes):
            cubic_p = valid_cubeCenters_xyz[i]

            #out_pts_flag = np.linalg.norm(pts - cubic_p, axis = 1) > radius
            #pts_out = pts[out_pts_flag]
            pts_out = pts

            line = t_p - cubic_p
            tt = np.dot(pts_out-cubic_p, line[:,:,None])[:,:,0]/np.sum(line * line, axis = 1)
            dist_p = np.linalg.norm(tt[:,:,None] * line[None,:,:] + cubic_p - pts_out[:,None,:], axis = 2)
            disp_line = np.linalg.norm(tt[:,:,None] * line[None,:,:], axis = 2)
            occlusion_flag = ((disp_line > radius) * (tt > 0) * (dist_p < 1.0 * radius)).sum(axis = 0)
            
            occlusion_viewPairs[i,:] = np.exp(-0.5 * occlusion_flag)

        return occlusion_viewPairs
    ...
    def viewPairSelection(self, 
        ...
        outputs:
            w_viewPairs: 
        """
        print('start using eNet to do viewPair selection')
        ttt = time.time()
        ...
        fe_viewPairs = np.empty((N_validCubes, N_viewPairs, 2 * D_embedding + 2), dtype = np.float32)
        ...
        #for _batch in utils.yield_batch_npBool(N_all = N_validCubes, batch_size = batchSize):
            ...
            features_viewPairs = np.concatenate([theta_viewPairs[_batch], d_viewPairs[_batch], _e_viewPairs], axis=-1).astype(np.float32).reshape((N_batch*N_viewPairs, N_features))
            
            #self.features_viewPairs = features_viewPairs

            fs = torch.from_numpy(features_viewPairs).to(self.device)
            ws = eNet(fs).detach().cpu().numpy()
            
            fe_viewPairs[_batch] = features_viewPairs.reshape((N_batch, N_viewPairs, -1))
            w_viewPairs[_batch] = ws.reshape((N_batch, N_viewPairs))


        if(use_occlusion and (pts is not None)):
            t_start = time.time()
            occlusion_views = self.occlusion_detect(cameraTs_np, cubeCenters_xyz[validCubes], pts, resol = resol)
            occlusion_viewPairs = occlusion_views[:,viewPairs].mean(axis = 2)
            w_viewPairs = w_viewPairs * occlusion_viewPairs
            print('occlusion detect cost: ', time.time() - t_start)
        # select N_argmax viewPairs
        #selected_viewPairs, selected_similNet_weight = self.argmaxN_viewPairs(viewPairs = viewPairs, w_viewPairs = w_viewPairs, N_argmax = N_viewPairs4inference)
        selected_viewPairs, selected_similNet_weight = self.argmaxN_viewPairs(viewPairs = viewPairs, w_viewPairs = w_viewPairs, features_viewPairs = fe_viewPairs,  N_argmax = N_viewPairs4inference)
        print('output features_viewPairs')
        print('view Pair selection cost:', time.time() - ttt)
...
class Volumn(Params):
    def __init__(self, resol = 1.0, model_num = 9, cube_D = None):
    ...
    def init_data(self, images_list = None, cameraPOs_np = None, cameraTs_np = None):
        ...
        self.initial_pts_xyz = None
        
    def init_volumn(self,images_list, cameraPOs_np, cameraTs_np, rejection = 'None', pts_xyz = None):
        ...
        if rejection is 'None':
        ...
        elif rejection is 'train_rejection':
            print('start train rejection')
            ttt = time.time()
            self.initial_pts_xyz = self.scene.readPointCloud_xyz(pointCloudFile = os.path.join(self.datasetFolder, self.stl_comb_name))
            self.cubes_param_np, self.cube_D_mm = self.scene.quantizePts2Cubes(pts_xyz = self.initial_pts_xyz, 
                                                                ...
                                                                cube_overlapping_ratio = self._cube_overlapping_ratio, 
                                                                BB = self.BB)
            print('end train rejection and cost: ', time.time()-ttt)

        elif (rejection is 'multiscale') and (pts_xyz is not None) :
            print('start multiscale rejection')
            ttt = time.time()
            #self.initial_pts_xyz = self.scene.readPointCloud_xyz(pointCloudFile = os.path.join(self.datasetFolder, self.stl_comb_name))
            self.initial_pts_xyz = pts_xyz
            self.cubes_param_np, self.cube_D_mm = self.scene.quantizePts2Cubes(pts_xyz = pts_xyz, 
                                                                resol = self.resol, 
                                                                cube_D = self._cube_D, \
                                                                cube_Dcenter = self._cube_Dcenter,
                                                                cube_overlapping_ratio = self._cube_overlapping_ratio, 
                                                                BB = self.BB)
            print('end multiscale rejection and cost: ', time.time()-ttt)
    ...
    def train_rejection(self, dataset):
        ...
        for i in range(cubic_num):
            ...
            flag_min = ((dataset.ply.pcd_xyz[::50] - loc_min) >=0)
            flag_max = ((loc_max - dataset.ply.pcd_xyz[::50]) >=0)
            flag = flag_min * flag_max
            flag_in = flag.prod(axis = 1)
            flag_all_in = flag_in.sum()
            #output = ((flag_all_in >= 100) and self.validCubes[i])
            output = (flag_all_in >= 1)
    ...
    def early_rejection(self, images_list = None, dataset = None):
        ...
        if(self._datasetName != 'MVS2d'):
            if(self._use_old_early_rejection):
                ...
                index = (self.img_w_cubesCenter >= (self.resol * self._cube_D * 3**0.5/3)) * (self.img_w_cubesCenter <= (image_w - self.resol * self._cube_D * 3**0.5/3)) *\
                        (self.img_h_cubesCenter >= (self.resol * self._cube_D * 3**0.5/3)) * (self.img_h_cubesCenter <= (image_h - self.resol * self._cube_D * 3**0.5/3))
                ...
                for i_view, image in enumerate(images_list):
                    for i_cubic in range(self.N_cubes):
                        ...
                        if((loc_h > (self._imgPatch_hw_size/2 + 1)) and (loc_h < (image_h - self._imgPatch_hw_size/2 - 1)) and (loc_w > (self._imgPatch_hw_size/2 + 1)) and (loc_w < (image_w - self._imgPatch_hw_size/2 - 1))):
                            ...
                            patch_image = image[(loc_h - self._imgPatch_hw_size/2):(loc_h + self._imgPatch_hw_size/2),(loc_w - self._imgPatch_hw_size/2):(loc_w + self._imgPatch_hw_size/2),:]
                            
                            bins = np.arange(0,257,8)
                            r_h,_ = np.histogram(patch_image[:,:,0], bins = bins, density = True)
                            g_h,_ = np.histogram(patch_image[:,:,1], bins = bins, density = True)
                            b_h,_ = np.histogram(patch_image[:,:,2], bins = bins, density = True)
                            image_hist = np.concatenate((r_h,g_h,b_h))

                            patch_image = scipy.misc.imresize(patch_image, (6,6))/256.0
                            self.patches_embedding[i_cubic, i_view] = np.concatenate((image_hist, patch_image.flatten()))
                
                self.dissimilarity = np.zeros((self.N_cubes, (self.N_views * (self.N_views-1)) / 2))

                self.validCubes = (index.sum(axis = 0) >= 2)
                self.validCubesIndex = [i for i, x in enumerate(self.validCubes) if x]
                self.N_validCubes = self.validCubes.sum()
                print('use hist')
                print('N_validCubes', self.N_validCubes)
            ...
            for i_view, image in enumerate(images_list):
                for i_cubic in range(self.N_cubes):
                    ...
                    if((loc >= self.D_embedding/6) and (loc <= (image_shape_0 - self.D_embedding/6))):
                        ...
                        #self.patches_embedding[i_cubic, i_view] = image[(loc - self.D_embedding/6):(loc + self.D_embedding/6), ]

            self.dissimilarity = np.zeros((self.N_cubes, (self.N_views * (self.N_views-1)) / 2))
            
            if(self.validCubes is None):
                self.validCubes = (index.sum(axis = 0) >= 5)
            else:
                self.validCubes = (index.sum(axis = 0) >= 5) * self.validCubes
            self.validCubesIndex = [i for i, x in enumerate(self.validCubes) if x]
            self.N_validCubes = self.validCubes.sum()
        
    def free_space(self):
        print('old self.N_validCubes',self.N_validCubes)
        validCubes_num = self.validCubes.shape
        rand_matrix = np.random.rand(validCubes_num[0])
        new_validCubes_empty = ((1-self.validCubes) * rand_matrix > 0.7)
        self.validCubes = new_validCubes_empty + self.validCubes
        self.validCubesIndex = [i for i, x in enumerate(self.validCubes) if x]
        self.N_validCubes = self.validCubes.sum()
        print('new self.N_validCubes', self.N_validCubes)
            
    def viewSelection(self,mode = 'train', eNet = None):
        ...
        if(mode == 'train'):
            ...
            self.viewPairs4Reconstr, self.embeddings = self.viewPairSelection.viewPairSelection( \
                eNet = eNet,
                cameraTs_np = self.cameraTs_np,  \
                e_viewPairs = self.patches_embedding,  \
                d_viewPairs = self.dissimilarity,  \
                ...
                batchSize = self._batchSize_viewPair_w,  \
                N_viewPairs4inference = self.N_viewPairs4inference[0], \
                viewPairs = self.viewPairs,
                use_occlusion = True,
                resol = self.resol,
                pts = self.initial_pts_xyz,)     # (N_validCubes, N_viewPairs4inference, 2), (N_validCubes, N_viewPairs4inference)

            #if params.__weighted_fusion is False:
            #    w_viewPairs4Reconstr[:] = 1.0 / N_viewPairs4inference


class Recorder(Params):
    def __init__(self, mode = 'none', train_rejection = True, file_root_volumn = "./inputs/DTU_MVS/model_class/", file_name = 'none'):
        super(Recorder, self).__init__()
        self.mode = mode
        self.file_root_volumn = file_root_volumn
        self.train_rejection = train_rejection
        self.init_data()
    ...
    def get_volumn(self, model_num):
        #file_root_volumn = "./inputs/DTU_MVS/model_class_new/"
        #file_name = 'volumn_resol:%s_%s.file'%(str(1),str(model_num).zfill(3))
        file_name = 'resol:%s_angle:%s_model:%s.file'%(str(round(self.resol,2)),str(self.thre_angle),str(model_num).zfill(3))
        with open(self.file_root_volumn + file_name, "rb") as f:
            volumn = pickle.load(f)
        print('load volumn success')
        return volumn
    ...
    def record_data(self, 
        ...
        self.cameraPOs_np = cameraPOs_np
        self.cameraTs_np = cameraTs_np
        if(self.mode == 'save_volumn'):

            #self.file_root_volumn = "./inputs/DTU_MVS/model_class_new/"
            #file_name = 'volumn_resol:1_%s.file'%str(model_num).zfill(3)
            file_name = 'resol:%s_angle:%s_model:%s.file'%(str(round(self.resol,2)),str(self.thre_angle),str(model_num).zfill(3))
            with open(self.file_root_volumn + file_name, "wb") as f:
                pickle.dump(volumn, f, pickle.HIGHEST_PROTOCOL)
            print('save success')
        else:
...
class Dataset(data.Dataset, Params):
    ...
    def init_data(self, model_num):
        ...
        if (not(self.recorder.record_flag(model_num)))or self.reconstruct: 
            ...
            if(self.recorder.mode == 'load_volumn'):
                ...
                self.recorder.record_flag(model_num)
        
                #self.images_list_dict[model_num] = images_list
                self.recorder.cameraPOs_np = self.cameraPOs_np
                self.recorder.cameraTs_np = self.cameraTs_np
                ...
                if(self.recorder.train_rejection):
            ...
            self.volumn = self.recorder.get_volumn(model_num)
            self.ply.init(min_point = self.volumn.cubes_param_np['xyz'][0], 
                         max_point = self.volumn.cubes_param_np['xyz'][-1],
                         resol = self.volumn.cubes_param_np['resol'][0])
...
class Dataset(data.Dataset, Params):
    ...
    def __init__(self, 
                 ...
                 transform = None):
        '''
        mode: load_volumn, init_volumn, save_volumn
        train_rejection: total, none
        '''
        super(Dataset, self).__init__()
        
        print('start new')
        
        self.load_modelSpecific_params(self._datasetName, model_num, cube_D = cube_D, sparsity = sparsity)
        self.mode = mode
        self.train_rejection = train_rejection
        self.data_file_name = data_file_name
        self.load_type = load_type
        self.type_volumn = type_volumn
        ...
        if(addition_name is not None):
            self.addition_name = addition_name
        if(resol is not None):
            self.resol = resol

        if(thre_angle is not None):
            self.thre_angle = thre_angle
        if(selection_method is not None):
            self.selection_method = selection_method
        if(viewPair_mode is not None):
            self.viewPair_mode = viewPair_mode
        if(N_viewPairs4inference is not None):
            self.N_viewPairs4inference = N_viewPairs4inference
        
        if(type_volumn == 'train'):
            self.file_root_volumn = self.file_root_volumn_train
        elif(type_volumn == 'test'):
            self.file_root_volumn = self.file_root_volumn_test
        else:
            raise ValueError('dataset type got wrong')
        
        self.init_tools(model_num)
        self.init_data(model_num, eNet)
        self.transform = transform
    ...
    def init_tools(self, model_num):
        self.camera = Camera()
        self.select = ViewPairSelection(selection_method = self.selection_method, thre_angle = self.thre_angle)
        self.image = Image()
        self.volumn = Volumn(resol = self.resol, model_num = model_num, cube_D = self._cube_D)
        self.cvc = CVC()
        self.ply = PLY2volumn(model_num, cube_D = self._cube_D)
        
    def record_data(self, model_num, volumn):
        
        #file_name = 'volumn_resol:1_%s.file'%str(model_num).zfill(3)
        #file_name = 'resol:%s_angle:%s_model:%s.file'%(str(self.resol),str(self.thre_angle),str(model_num).zfill(3))
        if(self.load_viewPair_mode != 'fixed'):
            file_name = 'resol:%s_rejection:%s_model:%s%s.file'%(str(self.resol),str(self.train_rejection),str(model_num).zfill(3), self.addition_name)
        else:
            file_name = 'resol:%s_angle:%s_rejection:%s_selection:%s_model:%s%s.file'%(str(self.resol),str(self.thre_angle),str(self.train_rejection),str(self.selection_method),str(model_num).zfill(3), self.addition_name)
        
        if(self.data_file_name is None):
            f = open(self.file_root_volumn + file_name, "wb")
            pickle.dump(volumn, f)
        else:
            f = open(self.data_file_name, "wb")
            pickle.dump(volumn, f)

        print('save success')
        print(self.file_root_volumn + file_name)
        
    def get_volumn(self, model_num):
        
        #file_name = 'volumn_resol:%s_%s.file'%(str(1),str(model_num).zfill(3))
        #file_name = 'resol:%s_angle:%s_model:%s.file'%(str(0.4),str(self.thre_angle),str(model_num).zfill(3))
        if(self.load_viewPair_mode != 'fixed' and self.load_type != 'old'):
            file_name = 'resol:%s_rejection:%s_model:%s%s.file'%(str(self.resol),str(self.train_rejection),str(model_num).zfill(3), self.addition_name)
        else:
            file_name = 'resol:%s_angle:%s_rejection:%s_selection:%s_model:%s%s.file'%(str(self.resol),str(self.thre_angle),str(self.train_rejection),str(self.selection_method),str(model_num).zfill(3), self.addition_name)
        
        if(self.data_file_name is None):
            f = open(self.file_root_volumn + file_name, "rb")
            volumn = pickle.load(f)
        else:
    ...
    def init_data(self, model_num, eNet = None):
        ...
        if(self.blur_ratio is not None):
            self.images_list = self.image.blurImgs(self.images_list, kernelRadius = self.blur_ratio  / 2)
        ...
        self.cameraTs_np = self.camera.cameraPs2Ts(cameraPOs = self.cameraPOs_np)
        print('images_list', self.images_list[0].shape)
        print('cameraTs_np', self.cameraTs_np.shape)
        print('start volumn operation')
        if(self.mode == 'debug'):
            self.volumn.init_volumn(self.images_list,
                                   self.cameraPOs_np,
                                   self.cameraTs_np)
            print('volumn init finished')

        elif(self.mode == 'init_volumn'):
            print('start generating')
            ttt = time.time()
            self.volumn.init_volumn(self.images_list,
                                   self.cameraPOs_np,
                                   self.cameraTs_np,
                                   rejection = self.train_rejection,
                                   pts_xyz = self.pts_xyz)
            print('volumn init finished')
            print('start earlt rejection')
            self.volumn.early_rejection(images_list = self.images_list)
            print('early rejection finished')

            print('resol', self.resol)
            if((self._datasetName != 'Middlebury') and (self.train_rejection == 'train_rejection') and False):
                self.ply.init(min_point = self.volumn.cubes_param_np['xyz'].min(axis = 0), 
                         max_point = self.volumn.cubes_param_np['xyz'].max(axis = 0),
                         resol = self.volumn.cubes_param_np['resol'][0])
                print('ply_init finished')
            #if(self.train_rejection != 'none'):
            #    
            #    print('train_rejection start')
            #    self.volumn.train_rejection(self)
            #    print('train_rejection finished')
            if(self.load_viewPair_mode == 'fixed'):
                print('start view selection')
                self.volumn.N_viewPairs4inference = self.N_viewPairs4inference
                self.volumn.viewPairSelection = ViewPairSelection(selection_method = self.selection_method, thre_angle = self.thre_angle)
                self.volumn.viewSelection(mode = self.viewPair_mode, eNet = eNet)
            print('volumn operation time cost: ', time.time() - ttt)
            
        elif(self.mode == 'save_volumn'):
            ...
            print('init volumn cost:', time.time() - ttt)
            t1 = time.time()
            ...
            self.ply.init(min_point = self.volumn.cubes_param_np['xyz'][0], 
                     ...
                     resol = self.volumn.cubes_param_np['resol'][0])
            print('ply_init finished')
            print('ply_init cost:', time.time() - t1)
            t1 = time.time()
            ...
            #    self.volumn.train_rejection(self)
            #    print('train_rejection finished')
            #    print('train_rejection cost:', time.time() - t1)
            #    t1 = time.time()

            if(self.type_volumn != 'none'):
                self.volumn.early_rejection(images_list = self.images_list)

                print('early rejection finished')
                print('early rejection cost:', time.time() - t1)
                t1 = time.time()
                ...
                if(self.load_viewPair_mode == 'fixed'):
                    ...
                    self.volumn.viewPairSelection = ViewPairSelection(selection_method = self.selection_method, thre_angle = self.thre_angle)
                    self.volumn.viewSelection(mode = self.viewPair_mode, eNet = eNet)
                    print('viewSelection cost:', time.time() - t1)
                    t1 = time.time()

            print('volumn operation time cost: ', time.time() - ttt)
            self.record_data(model_num, self.volumn)
                            
        elif(self.mode == 'load_volumn'):
            self.volumn = self.get_volumn(model_num)
            self.volumn._batchSize_viewPair_w = self._batchSize_viewPair_w 
            print(self.volumn._batchSize_viewPair_w)
            self.volumn.viewPairSelection.device = torch.device("cuda" if (True and torch.cuda.is_available()) else "cpu")

            if(self.type_volumn == 'test'):
                t1 = time.time()
                self.volumn.early_rejection(images_list = self.images_list)
            ...
            if(self.load_viewPair_mode != 'fixed'):
                ttt = time.time()
                if(self.free_space):
                    self.volumn.free_space()
                self.volumn.N_viewPairs4inference = self.N_viewPairs4inference
                self.volumn.viewPairSelection = ViewPairSelection(selection_method = self.selection_method, thre_angle = self.thre_angle)
                self.volumn.viewSelection(mode = self.viewPair_mode, eNet = eNet)
                print('volumn view selection cost', time.time() - ttt)
    ...
    def transform_label(self, idx):
        ...
        if((self._datasetName == 'Middlebury') or (self.train_rejection != 'train_rejection') or True):
            return np.zeros((1,self._cube_D,self._cube_D,self._cube_D))
        else:
            idx_validCubes = self.volumn.validCubesIndex[idx]
            self.surface = self.ply.gen_idx(voxel_point = self.volumn.cubes_param_np['xyz'][idx_validCubes],
                                        resol = self.volumn.cubes_param_np['resol'][idx_validCubes])
